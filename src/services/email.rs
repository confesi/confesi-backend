...
#[get("/verify/{token}/")]
pub async fn verify_link(db: web::Data<Database>, user: AuthenticatedUser) -> ApiResult<(), ()> {
	// todo: when verifying, do one last check to ensure the email isn't already in use (potential race condition between 2 people verifying the same email)
	success(())
}
